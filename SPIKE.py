...
SEED = 42  # torna a simulacao inteira reprodutivel
...
class Evento:
    ...
    minuto: int    # tempo logico simulado (minutos) do instante do evento
    tipo: str      # "recarga" ou "debito"
    valor: float


def gerar_eventos_reais(rng):
    """Gera a linha do tempo 'verdadeira': o que de fato aconteceu, na ordem
    causal correta, como se cada onibus estivesse sempre online. E o
    oraculo contra o qual comparamos as reconciliacoes."""
    ...
    minuto = 1
    for _ in range(40):
        c = rng.choice(cartoes)
        b = rng.choice(onibus)
        minuto += rng.randint(1, 6)
        novo(c, b, minuto, "debito", TARIFA)

    # Injeta 2 casos de uso duplicado: o mesmo cartao validado em dois
    # onibus diferentes, ambos offline, dentro da janela de suspeita.
    # Isso e exatamente o que o validador embarcado, sozinho, nao consegue
    # impedir: ele responde em 300 ms sem rede e nao enxerga o outro onibus.
...
def simular_rede_instavel(eventos, rng):
    """Simula o que e externo: a sincronizacao dos validadores com a central
    por 4G intermitente. Retransmissao por confirmacao perdida gera
    duplicatas; onibus diferentes sincronizam em momentos diferentes, entao
    a ordem de chegada na central nao e a ordem causal em que os eventos
    de fato aconteceram."""
...
class ReconciliadorEventos:
    """O que a decisao da matriz propoe: consumidor idempotente (dedup por
    event_id) com reconciliacao causal por cartao (buffer ate a sequencia
    fechar) e trilha imutavel para auditoria e deteccao de uso duplicado."""

    def __init__(self):
        self.saldo = defaultdict(float)
        self.processados = set()
        self.esperado = defaultdict(lambda: 1)
        self.buffer = defaultdict(dict)
        self.log = []
        self.duplicatas_filtradas = 0
        self.eventos_reordenados = 0
    ...
    def reconstruir_do_log(self):
        """Prova de auditabilidade: reconstroi o saldo do zero a partir so
        da trilha imutavel -- sem depender do estado em memoria."""
    ...
    def detectar_uso_duplicado(self, janela_min=JANELA_SUSPEITA_MIN):
        """Varre a trilha imutavel por cartao e aponta pares de debito em
        onibus diferentes dentro da janela de suspeita. Prova que, mesmo
        sem impedir o uso duplicado no instante da validacao (o validador
        estava offline), o sistema descobre isso depois, pela trilha."""
        por_cartao = defaultdict(list)
        for ev in self.log:
            if ev.tipo == "debito":
                por_cartao[ev.cartao].append(ev)
        achados = []
...
def main():
    ...
    eventos = gerar_eventos_reais(rng)
    entregues = simular_rede_instavel(eventos, rng)
    verdade = saldo_verdadeiro(eventos)
    ...
    for cartao in sorted(verdade):
        v = verdade[cartao]
        i = idem.saldo[cartao]
        n = ingenuo.saldo[cartao]
        ok = abs(v - i) < 1e-9
        todos_ok = todos_ok and ok
        marca = "OK" if ok else "FALHOU"
        print(f"{cartao:<8}{v:>14.2f}{i:>30.2f}{n:>16.2f}   [{marca}]")
    print("-" * 72)

    bate = reconstruido == dict(idem.saldo)
    print()
    print(f"Reconstrucao a partir da trilha imutavel bate com o saldo em memoria: {'SIM' if bate else 'NAO'}")

    divergentes = sum(1 for c in verdade if abs(verdade[c] - ingenuo.saldo[c]) > 1e-9)
    print()
    print(f"Cartoes onde o reconciliador ingenuo diverge do saldo real: {divergentes} de {len(verdade)}")
    ...
    print()
    print("Uso duplicado da mesma passagem em onibus diferentes, detectado na")
    ...
    if achados:
        for cartao, a, b in achados:
            print(f"  - {cartao}: debito em {a.onibus} (min {a.minuto}) e "
                  f"{b.onibus} (min {b.minuto}) -- diferenca de {b.minuto - a.minuto} min")
    else:
        print("  (nenhum caso na simulacao)")
    ...
    if todos_ok and bate and divergentes > 0 and achados:
        ...
        print("RESULTADO: FAIL -- ver detalhes acima.")
    print("=" * 72)
